...
**Две стратегии, а не одна.** Код приложения (`html`/`css`/`js`) отдаётся
по схеме **Network-First**, статика (иконки) — по **Cache-First**:

```js
// код приложения: онлайн всегда свежий, офлайн — из кеша
try { return await fetch(request); }
catch { return await caches.match(request); }
```

Сплошной Cache-First обошёлся дорого: исправления не доходили до
устройства, пока вручную не сменится имя кеша, и пользователь неделю
видел старую версию. Клиент дополнительно слушает `controllerchange`
и перезагружает страницу, когда новый воркер берёт управление.
...
### Три режима ввода
Один движок расчёта обслуживает три режима — отличается только то,
откуда берётся масса основы и доли:

| Режим | Основа | Доли |
|---|---|---|
| Масса | вводит маляр | из техпаспорта |
| Площадь | выводится из расхода | из техпаспорта |
| Свой | вводит маляр | вводит маляр |

Общая функция `buildMix(mat, opts)` принимает `mode` и ветвится только
в двух местах — при получении основы и при выборе источника долей.
Никакого дублирования логики между режимами нет.

### Добавки: сверх или из разбавителя
Замедлитель и ускоритель по умолчанию идут **сверх** отвердителя
и разбавителя (указание производителя). Галочка «Вычесть из разбавителя»
переключает на вторую схему — добавки забираются из доли разбавителя:
```
доляРазбавителя = вычитать
  ? max(0, заданная − доляЗамедлителя − доляУскорителя)
  : заданная
```
Галочка отображается, только когда есть что вычитать, и подписывается
текущими значениями, чтобы маляр видел, о чём речь.

### Расчёт по площади (обратный, но с тем же инвариантом)
Расход г/м² в каталоге стоит в строке «Способ нанесения» — это готовая
рабочая смесь целиком (где имеется в виду только основа, каталог пишет
«часть А»):
```
смесь     = площадь × расход
основа    = смесь / (1 + доляОтвердителя/100 + доляРастворителя/100 + доляУскорителя/100)
компонент = основа × (доля / 100)
запас     = смесь × запас%          ← отдельной строкой, в основу не входит
итого     = смесь + запас
```
Инвариант «всё от основы» при этом сохраняется: делитель лишь подбирает
основу так, чтобы сумма совпала с расходом.

**Урок:** первую версию этой формулы вывели обратным счётом из одного числа
в примере пользователя. Число совпало, но трактовка оказалась неверной.
Проверка по первоисточнику (сканы каталога) исправила расчёт —
совпадение одной цифры не заменяет чтения документации.

Запас не закладывается в основу: компоненты считаются на чистую площадь,
а запас берётся от итоговой суммы и выводится отдельной строкой. Тогда
числа компонентов не «плывут» при смене запаса:
```
запас = смесь × запас%
итого = смесь + запас
```
